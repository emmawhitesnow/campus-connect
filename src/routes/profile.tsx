...
import { Settings, Pencil, MessageSquare, Phone, Heart, Calendar, LogOut, Lock } from "lucide-react";
...
export const Route = createFileRoute("/profile")({
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Profile — Orbit" },
      { property: "og:description", content: "Celebrate the people and places that make your week." },
    ],
  }),
  ...
  return (
    <div>
      <ScreenHeader
        title="Profile"
        back="/"
        right={<button className="text-primary" aria-label="Settings"><Settings size={22} /></button>}
      />
      <div className="px-5">
        <div className="flex items-center gap-4 mt-2">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="mt-2 flex gap-4 text-sm">
              <span className="text-primary"><b>{me.connections}</b> <span className="text-muted-foreground font-normal">connections</span></span>
              <span className="text-primary"><b>{me.clubs}</b> <span className="text-muted-foreground font-normal">clubs</span></span>
            </div>
          </div>
        </div>
        ...
        <div className="mt-3 grid grid-cols-3 gap-2">
          <StatRing value={92} unit="%" label="show up rate" percent={92} />
          <StatRing value={42} label="people met" percent={68} />
          <StatRing value={7} label="clubs attended" percent={87} />
        </div>
          ...
          {closestFriends.map((f) => (
            <div key={f.id} className="flex items-center gap-3 p-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[11px] text-muted-foreground">{f.mutuals} mutuals</p>
        ...
        <div className="mt-3 rounded-3xl bg-card p-2 divide-y divide-border mb-6">
          {myClubs.map((c) => (
            <div key={c.id} className="flex items-center gap-3 p-3">
              <div className="size-12 rounded-full bg-primary/10 text-primary grid place-items-center font-bold">
                {c.name[0]}
              </div>
              <div className="flex-1 min-w-0">
